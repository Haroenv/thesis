...
%% find react setState
All raw parameters are being rewritten in place, similar to how {\tt setState} works in React~\cite{react_setstate}~. A function called {\tt refineRaw} is exposed on a state container. This function expects a function as an argument. It receives the previous state for the refinements as its parameter, and is expected to return the new state object, based on whatever happened that needs to change.
