...
When Algolia pivoted in 2013 to a \acrshort{saas} product~\cite{algolia-blog-saas}, API clients~\cite{algolia-blog-lauch} for six programming languages were immediately available. JavaScript has always been important because of the focus on speed that Algolia has.
...
Before being a separate concept, the ``Helper'' was an integral part of the JavaScript API client~\cite{algolia-blog-js-client}~. In 2015 it was moved out as a standalone \gls{library} with as its main feature handling state of a search experience.
...
InstantSearch.js was the solution to keeping from people building the same solutions over and over. Its goal is to bridge the gap between the JavaScript helper, which has a very extensive API and users, who don't always have time to grok all the options. 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth, height=0.2\textheight, draft]{../assets/is-js-state.pdf}
...
In contrast to InstantSearch.js, not the JavaScript Helper is being used to communicate with the Algolia \acrshort{rest} API, but a new small layer on top of it. As described in more detail in section \ref{ssec:Subscribing}, Vue will take in account the {\tt getters} of an Object. These getters are written as a function, which will be called periodically by the Vue \gls{library}. 
...
In the future, Vue InstantSearch will be the first \gls{library} to try the experimentation described in chapter \ref{chp:execution}, since it explicitly made it close to the API of InstantSearch Core.

As an experiment with Lucas Bonomi, a way to use the connectors of InstantSearch.js with Moon --- a \gls{library} very close to Vue in it's API --- has been made internally. This proves that connectors will be a way to make InstantSearch available to various other programming libraries in the future.
...
Since InstantSearch Core, as described in section \ref{sec:organizing_state}, maps the state of the each component to its \gls{attribute} name. In there it has a type and a value, which makes it very easy to know which widgets to render.
